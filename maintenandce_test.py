# -*- coding: utf-8 -*-
"""
Created on Fri Jul 29 14:30:20 2022
...
"""
###########################################################################################
#JSON_PARSER MAINTENANCE(CLOSED)!!!!!!!!!!!!
###########################################################################################
import json
import ast
...
def clean_data(data_name):    
    ...
    with open("Documents2scrape\\JSON_FILES\\"+data_name+'_cleaned.json',"w",encoding='utf-8') as outfile:
        outfile.write(extracted_json)    
#clean_data("formatted_page_1")

def json_to_csv(json_data):
    pdobj=pd.read_json("Documents2scrape\\JSON_FILES\\"+json_data+'.json',orient='index')
    pdobj.to_csv('Documents2scrape\\JSON_FILES\\CSV_FILES\\'+json_data+'(csv).csv',index=True)
clean_data('formatted_page_1')         
json_to_csv('formatted_page_1_cleaned') 